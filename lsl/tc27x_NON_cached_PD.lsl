...
derivative tc27x
{
        ...
        memory dspr0 // Data Scratch Pad Ram
        {
                ...
                size = 0x4000;
                type = ram;
                map (dest=bus:tc0:fpi_bus, dest_offset=0xd0000000, size=0x4000, priority=8);
                map (dest=bus:sri, dest_offset=0x70000000, size=0x4000);

                //map (dest=bus:tc0:fpi_bus, dest_offset=0xd0000000, size=112k, priority=8);
                //map (dest=bus:sri, dest_offset=0x70000000, size=112k);
        }
        memory dspr0_aftCal // Data Scratch Pad Ram after the segment reserved for calibration
        {
                mau = 8;
                size = 0x16000;
                type = ram;
                map (dest=bus:tc0:fpi_bus, dest_offset=0xd0006000, size=0x16000, priority=8);
                map (dest=bus:sri, dest_offset=0x70006000, size=0x16000);
        }
        ...
        memory dspr1 // Data Scratch Pad Ram
        {
                ...
                size = 0x4000;
                type = reserved ram;
                map (dest=bus:tc1:fpi_bus, dest_offset=0xd0000000, size=0x4000, priority=8);
                map (dest=bus:sri, dest_offset=0x60000000, size=0x4000);

                //map (dest=bus:tc1:fpi_bus, dest_offset=0xd0000000, size=120k, priority=8);
                //map (dest=bus:sri, dest_offset=0x60000000, size=120k);
        }

        memory dspr1_aftCal // Data Scratch Pad Ram after the segment reserved for calibration
        {
                mau = 8;
                size = 0x18000;
                type = reserved ram;
                map (dest=bus:tc1:fpi_bus, dest_offset=0xd0006000, size=0x18000, priority=8);
                map (dest=bus:sri, dest_offset=0x60006000, size=0x18000);
        }
        ...
        memory pflash0
        {
                ...
                size = 2M;
                type = rom;
                map     cached (dest=bus:sri, dest_offset=0x80000000, reserved, size=2M);
                map not_cached (dest=bus:sri, dest_offset=0xa0000000,           size=2M);
        }
        ...
        memory pflash1
        {
                ...
                size = 2M;
                type = rom;
                map     cached (dest=bus:sri, dest_offset=0x80200000, reserved, size=2M);
                map not_cached (dest=bus:sri, dest_offset=0xa0200000,           size=2M);
        }
        ...
        memory lmuram
        {
                ...
                type = ram;
                priority = 2;
                //map     cached (dest=bus:sri, dest_offset=0x90000000, reserved, size=32k);
